...
def line_plot_all_algos(
    ...
    A = df.select_dtypes(include=[np.number]).clip(0, 80)
    if A.empty:
    ...
    ax.set_ylabel("Final Agent Counts")
    ...
    fig.suptitle(title, y=0.8)   # place title just a bit above legend
    
    ax.legend(
        loc="upper center",
        bbox_to_anchor=(0.5, 1.15),  # legend just under title
        ncol=len(algos),
        frameon=False
    )
...
line_plot_all_algos(
    "Final_agent.csv",
    title="Final Agent Counts by Algorithm",
